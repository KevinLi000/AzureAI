                ...
                if is_multi_column:
                    ...
                    for block in blocks:
                        ...
                        if block["type"] == 0:  # 文本类型
                            ...
                            # 如果Y坐标相差超过行间距阈值，或者水平位置有明显变化，创建新段落
                            # 增强段落识别准确性
                            new_paragraph_needed = (current_y == -1 or 
                                                  (abs(block_y - current_y) > 12) or  # 增加阈值以更准确检测段落
                            ...
                            if new_paragraph_needed:
                                ...
                                # 设置段落对齐方式和缩进
                                try:
                                    # 修复错误: _detect_paragraph_format 可能返回多个值
                                    ...
                                    # 应用段落缩进
                                    if left_indent > 0:
                                        current_paragraph.paragraph_format.left_indent = Cm(left_indent / 28.35)  # 点转换为厘米
                                except Exception as e:
                                    print(f"设置段落格式时出错: {e}")
                                    # 使用默认格式
            ...
            for merge_info in fixed_merged_cells:
                ...
                if (0 <= start_row < rows and 0 <= start_col < cols and
                    ...
                    if end_col > start_col:
                        cell.merge(word_table.cell(start_row, end_col))            # 填充内容并应用样式
